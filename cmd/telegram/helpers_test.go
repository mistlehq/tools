...
func setupCommandEnvironment(t *testing.T) Environment {
	...
	upstreamBaseURL := strings.TrimSuffix(getRequiredEnv(t, "TELEGRAM_TEST_UPSTREAM_BASE_URL"), "/")
	token := getRequiredEnv(t, "TELEGRAM_TEST_BOT_TOKEN")

	proxy, err := testproxy.Start(testproxy.Config{
		UpstreamBaseURL:   upstreamBaseURL,
		AuthMode:          testproxy.AuthModePathSegmentPrefix,
		SegmentPrefix:     "bot",
		SegmentCredential: token,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	return Environment{"TELEGRAM_BASE_URL": proxy.BaseURL}
}
